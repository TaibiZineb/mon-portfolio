...
import { Code2, MonitorCheck,  Sun, Moon } from "lucide-react";
    ...
    <nav className="bg-[var(--primary)] shadow-lg text-white p-4 dark:text-black">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <ul className="hidden md:flex gap-6">
          <li>
            <Link to="/"className={location.pathname === "/" && !location.hash ? activeClass : normalClass}>
              Accueil
            </Link>
          </li>
          ...
          <li>
            <Link to="/skills" className={location.pathname === "/skills" ? activeClass : normalClass}> Compétences
            </Link>
          </li>
          <li>
        ...
        <div className="flex items-center gap-4">
          <div onClick={() => setDarkMode(!darkMode)} className={`relative w-14 h-7 flex items-center rounded-full p-1 cursor-pointer transition ${darkMode ? "bg-[var(--accent)]" : "bg-gray-400"}`}>
            {/* Lune à gauche */}
            <Moon size={14} className="absolute left-1 text-white" />
            {/* Soleil à droite */}
            <Sun size={14} className="absolute right-1 text-yellow-300" />
            {/* Bouton */}
            <div className={`bg-white w-5 h-5 rounded-full shadow-md transform transition ${darkMode ? "translate-x-7" : "translate-x-0"}`}/>
          </div>
          {/* Bouton hamburger (mobile) */}
          <button className="md:hidden text-white dark:text-black focus:outline-none" onClick={() => setIsOpen(!isOpen)}>
          {/* Icône hamburger */}
